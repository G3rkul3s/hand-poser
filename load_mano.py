...
def create_joint_armature(hand, joint_positions, bone_names, bone_parents):
...
def load_mano_hand(hand: str):
    ...
    # === Create base mesh ===
    obj = create_mano_mesh(f"MANO_{hand}_Hand_mesh", v_template, faces)

    # === Add shape keys ===
    add_shape_keys(obj, shapedirs, v_template)
    
    # === Add armature ===
    arm = create_joint_armature(hand, joints, BONE_NAMES, BONE_PARENTS)
    obj.parent = arm
    
    arm_mod = obj.modifiers.new(name="ArmatureDeform", type='ARMATURE')
    arm_mod.object = arm
    assign_skinning_weights(obj, weights, BONE_NAMES)
